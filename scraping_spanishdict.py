...
word_dict = infinidict()

verbs = ["ir", "hacer", "venir", "hablar"]
        ...
        for content_row in content_rows:
            ...
            for j, content_column in enumerate(content_columns):
                ...
                if len(word_div) > 0:  # if the word is not missing
                    ...
                    word_dict[mood][tenses[j]][pronoun][verb] = {"spanish": in_spanish, "english": in_english}

# convert the defaultdict to normal dict:
word_dict = json.loads(json.dumps(word_dict))

print(word_dict)

# save to json files:
with open(f'dictionary.json', 'w') as outfile:
        ujson.dump(word_dict, outfile)